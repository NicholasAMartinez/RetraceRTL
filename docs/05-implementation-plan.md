...
Based on the existing navigation modes in the repository, `RetraceRtl` will inherit from `NavigatorMode` and `ModuleParams`.

```cpp
class RetraceRtl : public NavigatorMode, public ModuleParams
```

For the prototype, most RetraceRTL-specific behavior will remain contained within the `RetraceRtl` class. This includes path storage, path recording, path validation, activation state, reverse traversal, and generation of the next position target.
...
Changes outside of the RetraceRTL implementation should be limited to the PX4 integration required to make the mode available. This will include registering the mode with Navigator, adding any required navigation-state handling, and adding parameter metadata when configurable RetraceRTL parameters are introduced.

The initial implementation will be developed incrementally:

1. Create the `RetraceRtl` class and integrate it with Navigator sufficiently for the mode lifecycle callbacks to execute.
2. Record valid local-position samples while RetraceRTL is inactive.
...
5. Allow RetraceRTL to activate using a valid stored path.
...
The proof of concept is complete when a simulated multicopter can fly a route, activate RetraceRTL, and visibly retrace the stored path in reverse using the existing PX4 navigation and position-control infrastructure.
